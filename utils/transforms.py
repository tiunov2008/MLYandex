...
def _highpass_kernel() -> torch.Tensor:
    kernel = torch.tensor(
        [
            [0.0, -1.0, 0.0],
            [-1.0, 4.0, -1.0],
...
def _compute_highpass(tensor: torch.Tensor, scale: float = 4.0, kernel: torch.Tensor = _HIGHPASS_KERNEL) -> torch.Tensor:
    if tensor.dim() != 3:
...
def _load_stats(stats_path: Path) -> tuple[float, float] | None:
...
def load_highpass_stats(stats_path: Path) -> tuple[float, float] | None:
...
def estimate_highpass_stats(
    ...
    if stats_path:
        ...
        if cached:
            ...
            if step_logger:
                step_logger.log(f"Loaded cached high-pass stats from {stats_path}")
            else:
    ...
    if stats_path:
        ...
        if step_logger:
            step_logger.log(f"Saved high-pass stats to {stats_path}")
        else:
...
def normalize_stats_with_highpass(hp_mean: float, hp_std: float) -> tuple[list[float], list[float]]:
